<?php
    ...
    // mengambil data tweet untuk diupdate
    $idtweet = $_GET['id'];
    ...
    if(isset($_POST['submit'])) {
        ...
        // pisahkan tag dengan isitweet
        $tags = explode("#", $isitweet);
        unset($tags[0]);
        $tags = implode(',', $tags); 

        // update tweet
        $sql = "UPDATE tweets SET judultweet='$judultweet', isitweet='$isitweet', tags='$tags' WHERE idtweet=$idtweet";
...
<body>
    ...
    <div class="container">
        <h1 style="margin-top: 50px">Update Tweet</h1>
        <hr>
        <form class="" method="POST" id="form_siswa">
            <div class="mb-3">
                <label for="judul" class="form-label">Judul Tweet</label>
                <input type="text" class="form-control" id="judul" name="judul" required autocomplete="off" value="<?php echo $judultweet ?>">
            </div>

            <div class="mb-3">
                <label for="isitweet" class="form-label">Isi Tweet</label>
                <textarea class="form-control" name="isitweet" id="isitweet" cols="30" rows="6" maxlength="250"><?php echo $isitweet ?></textarea>
            </div>

            <div class="mb-3">
                <button class="btn btn-primary" type="submit" name="submit">Update Tweet</button>
            </div>
        </form>
    </div>
</body>
